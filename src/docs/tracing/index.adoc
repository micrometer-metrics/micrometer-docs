...
It also provides a tracing extension to Micrometer's `TimerRecordingHandler`. Whenever a `Timer.Sample` is used, a corresponding span will be created, stared, stopped and reported.
...
You should add a tracing bridge you want to use, such as `micrometer-tracing-bridge-brave` or `micrometer-tracing-bridge-otel` and span exporters / reporters. In case of adding a bridge the `micrometer-tracing` library is added transitively.
...
include::tracers.adoc[leveloffset=+1]

== Supported Reporters

include::reporters.adoc[leveloffset=+1]
