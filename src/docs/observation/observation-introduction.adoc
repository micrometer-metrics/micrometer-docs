...
example of such a key value (such as `/example/user1234`, `/example/user2345` etc.). *Low cardinality* means that a key value will have a bounded number of possible values. A *templated* HTTP URL is a good example of such a key value (such as `/example/{userId}`).
...
-----
include::../../../samples/src/test/java/io/micrometer/docs/observation/ObservationTestingTests.java[tags=example,indent=0]
-----
