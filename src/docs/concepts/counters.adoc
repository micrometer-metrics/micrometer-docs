...
TIP: Never count something you can time with a `Timer` or summarize with a `DistributionSummary`! Both `Timer` and `Distribution` always publish a count of events in addition to other measurements.
