Timers are intended for measuring short-duration latencies and the frequency of such events. All implementations of `Timer` report at least the total time and the count of events as separate time series, and many implementations such as Prometheus can optionally report distribution (e.g., latency histogram buckets or latency percentile buckets). While you can use timers for other use cases, note that negative values are not supported, and recording many longer durations could cause overflow of the total time at `Long.MAX_VALUE` nanoseconds (292.3 years).
...
The percentile and histogram buckets (by `publishPercentiles`, `publishPercentileHistogram`, and `serviceLevelObjectives`) are also time windowed except for cumulative implementations such as Prometheus.
...
-----
include::../../../samples/src/test/java/io/micrometer/docs/metrics/TimedAspectTest.java[tags=resolvers,indent=0]

include::../../../samples/src/test/java/io/micrometer/docs/metrics/TimedAspectTest.java[tags=meter_tag_annotation_handler,indent=0]
-----
...
[source,java,subs=+attributes]
-----
include::../../../samples/src/test/java/io/micrometer/docs/metrics/TimedAspectTest.java[tags=interface,indent=0]
-----
...
include::../../../samples/src/test/java/io/micrometer/docs/metrics/TimedAspectTest.java[tags=example_value_to_string,indent=0]

// Example for calling the provided <ValueResolver> on the parameter
include::../../../samples/src/test/java/io/micrometer/docs/metrics/TimedAspectTest.java[tags=example_value_resolver,indent=0]

// Example for calling the provided <ValueExpressionResolver>
include::../../../samples/src/test/java/io/micrometer/docs/metrics/TimedAspectTest.java[tags=example_value_spel,indent=0]
-----
