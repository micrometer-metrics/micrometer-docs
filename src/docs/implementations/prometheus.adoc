...
Instead of using the `job` tag to distinguish different applications, this dashboard makes use of a common tag called `application` applied to every metric. You can apply the common tag with:
