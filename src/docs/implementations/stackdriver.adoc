...
MeterRegistry registry = StackdriverMeterRegistry registry = StackdriverMeterRegistry.builder(stackdriverConfig).build();
----
...
management.metrics.export.stackdriver:
    ...
    # You will probably want disable Stackdriver Monitoring publishing in a local development profile.
    enabled: true
