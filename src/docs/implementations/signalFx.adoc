...
image::signalfx-timer-latency-query.png[SignalFx-aggregable latency query]
...
.Timer latency.
image::signalfx-timer-latency.png[SignalFx-rendered timer]

To generate a throughput chart, use the `${name}.count` signal:

.Timer throughput.
image::signalfx-timer-throughput.png[SignalFx-rendered timer throughput]

To generate a plot of client-side percentiles, use the `${name}.percentiles` signal:

.Timer Percentiles.
image::signalfx-timer-percentiles.png[SignalFx-rendered percentiles]
...
image::signalfx-timer-sla.png[SignalFx-rendered SLO boundaries]
