...
The default proxy configuration will push metrics to a Wavefront proxy sitting on localhost:7828.

NOTE: If publishing metrics to a Wavefront proxy, the URI must be expressed in the form proxy://HOST:PORT.

== Graphing

This section serves as a quickstart to rendering useful representations in Wavefront for metrics originating in Micrometer. See the http://somewhere[Wavefront docs] for a far more complete reference of what is possible in Wavefront.
