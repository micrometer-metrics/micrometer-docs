...
that allow you to query for the latency at any percentile using `hs()` queries. For example, you can
...
<<wavefront-histograms>> below.

If `publishPercentileHistogram` is disabled, the Wavefront `Timer` produces several
time series:

1. `${name}.avg` - Mean latency across all calls.
2. `${name}.count` - Total number of all calls.
3. `${name}.sum` - Total time of all calls.
3. `${name}.max` - Max latency over the publishing interval.
4. `${name}.percentiles` - Micrometer calculated percentiles for the publishing interval. These
cannot be aggregated across dimensions!

These time series can be used to generate a quick view of latency in Wavefront:

.Timer latency.
image::img/wavefront-timer-latency.png[Wavefront-rendered timer]

The chart above demonstrates the average latency (`rate(ts(timer.sum))/rate(ts(timer.count))` in
